        ...
        @foreach ($menus as $menu)
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute bottom-11 w-full mt-2 origin-top-right rounded-md shadow-lg md:w-48">
                        <div class="px-2 py-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700">
                            @foreach ($menu->getChild() as $child)
                                <a class="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg text-gray-700 dark:text-gray-300 md:mt-0 hover:text-gray-900 dark:hover:text-white focus:text-gray-900 dark:focus:text-white hover:bg-gray-100 dark:hover:bg-gray-700 focus:bg-gray-100 dark:focus:bg-gray-700 focus:outline-none transition-colors" href="{{ Util::localizeUrl($child->link) }}" target="{{ $child->target }}">{!! __($child->name) !!}</a>
                            @endforeach
                        </div>
                    </div>
                </div>
            @else
                @if ($menu->parent_id === null)
                    <a href="{{ Util::localizeUrl($menu->link) }}" class="px-3 py-2 text-sm font-semibold text-left bg-transparent text-white {{ url()->current() === Util::localizeUrl($menu->link) ? "bg-white bg-opacity-25" : "" }} rounded-lg md:w-auto md:inline hover:bg-white hover:bg-opacity-10 focus:outline-none transition-colors" target="{{ $menu->target }}">{!! __($menu->name) !!}</a>
                @endif
            @endif
        @endforeach

        @if (Auth::check() && Auth::user()->role == 7)
            <a href="{{ route("admin") }}" target="_blank" class="px-3 py-2 text-sm font-semibold text-left bg-transparent border-dashed border-2 border-red-500 text-red-500 rounded-lg md:w-auto md:inline hover:bg-red-900 hover:text-white transition-colors">{{ __("Admin") }}</a>
        @endif

        @if (config("app.settings.user_registration.enabled") && ! Auth::check())
            <a href="{{ route("login") }}" class="px-3 py-2 text-sm font-semibold rounded-lg md:w-auto md:inline bg-secondary text-white hover:bg-secondary/90 transition-colors">{{ __("Login") }}</a>
        @endif
