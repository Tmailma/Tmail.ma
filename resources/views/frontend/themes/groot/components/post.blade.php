<main class="post flex-1 p-5 bg-white dark:bg-gray-800 rounded-md">
        ...
        <div class="relative z-10 flex flex-col justify-end h-full p-6 md:p-8 lg:p-10">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                @foreach ($post->categories as $category)
                    <a href="{{ Util::localizeUrl(route("blog.category", $category->slug)) }}" class="px-3 py-1 bg-gray-900 dark:bg-gray-700 text-white dark:text-gray-300 text-sm font-medium rounded-full hover:bg-gray-800 dark:hover:bg-gray-600 transition-colors">{{ $category->name }}</a>
                @endforeach
            </div>
    ...
    @if (config("app.settings.disqus.enable"))
        <div id="disqus_thread" class="mt-8 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg"></div>
        <script>
            (function () {
